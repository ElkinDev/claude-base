...
function Assert-Forwarded {
    ...
    Assert-Regex $Out '(?m)^CLAUDE_CODE_AUTO_COMPACT_WINDOW=\(unchanged\)\r?$' "${What}: the window is untouched"
...
Assert-Regex $out '(?m)^CLAUDE_CODE_AUTO_COMPACT_WINDOW=\(unchanged\)\r?$' '-w leaves the window alone'
