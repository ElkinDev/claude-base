...
A session can be opened with a role, `cc work -o orchestrator`, `-o analyst`, `-o lane` (the default) or `-o research`. The role does four things and nothing else, and the fourth, the seat, is what turns a role into a chair. The launcher does all four.
...
A seated session is never continued with `-c`. Profiles share the projects junction, so `-c` and `--continue` load the most recent conversation of the folder, which may belong to the other seat, and appending the right seat text to it does not stop a conversation that already holds a chair. The launcher refuses `-c`, `--continue` and `--continue=<id>` outright: it prints one line and exits without opening a session. Resume a seat with `-- -r` and the picker, `-- -r <session id>` or `-- --resume=<session id>`.
