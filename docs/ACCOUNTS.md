...
It appends the seat. When `<kit home>\seats\<role>.md` exists, the launcher passes `--append-system-prompt-file` with that file, so a session opened with a seat knows which board it drives, what it reads first in the morning and what it owes before the day closes, without a human typing any of it. Appended, never substituted: `--agent` replaces the default system prompt and takes the environment block, the model identity and the memory instructions with it. A role with no seat file, `lane` and `research` today, launches exactly as it did before. The seat files ship with the kit and the hooks already read `CLAUDE_ROLE`; wiring the append into `claude-account.ps1`, with `analyst` in the role list, the session name applied on a fresh launch only, and the refusal below, is its own item. `docs/SEATS.md` holds the mechanism, the launch lines and that deferral.

A seated session is never continued with `-c`. Profiles share the projects junction, so `-c` and `--continue` load the most recent conversation of the folder, which may belong to the other seat, and appending the right seat text to it does not stop a conversation that already holds a chair. Resume a seat with `-- -r` and the picker, or `-- -r <session id>`.
