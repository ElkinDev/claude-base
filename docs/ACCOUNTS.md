...
A session can be opened with a role: `cc work -Role orchestrator`, `-Role analyst`, `-Role lane` (the default) or `-Role research`. `-o` is the alias, and it is worth writing `-Role` in full for the seated roles, because a wrapper that forwards flags it does not know straight to `claude` can swallow the short form and leave you with no role and no error to read. The role does four things and nothing else, and the fourth, the seat, is what turns a role into a chair. The launcher does all four.

It sets the context cap. Every role but `research` runs with `CLAUDE_CODE_DISABLE_1M_CONTEXT=1`, so the window stays at 200k and compacts on the schedule the kit's hooks expect; `research` is the only uncapped role, for a session that reads far more than it writes. The variable is set with `$env:` in the window the script opens, never globally.
...
It names the session. When `-o` is given explicitly, the script passes `claude --name <role>` so the prompt box, the resume picker and the transcript all say `orchestrator` or `research` whatever pane number the multiplexer hands out. It is applied only when the role was typed on purpose: a plain `cc work -- -c` must not rename the orchestrator it resumes into `lane`. A seated role is named on a fresh launch only, and with the minute, `<role>-MMdd-HHmm`, so the picker separates today's chair from yesterday's; a resumed chair keeps the name it already carries, and `-c` is refused rather than renamed. Pass `-- --name x` to choose another name.

It appends the seat. When `<kit home>\seats\<role>.md` exists, the launcher passes `--append-system-prompt-file` with that file, so a session opened with a seat knows which board it drives, what it reads first in the morning and what it owes before the day closes, without a human typing any of it. Appended, never substituted: `--agent` replaces the default system prompt and takes the environment block, the model identity and the memory instructions with it. `orchestrator` and `analyst` are the seated roles; a role with no seat file launches as it always did, and a missing file costs one printed line, never the session. On a machine without the launcher the seat is typed by hand, `CLAUDE_ROLE=orchestrator claude --append-system-prompt-file ~/.claude/seats/orchestrator.md` in a POSIX shell and `$env:CLAUDE_ROLE='orchestrator'; claude --append-system-prompt-file "$env:USERPROFILE\.claude\seats\orchestrator.md"` in PowerShell. `docs/SEATS.md` holds the mechanism, the seats directory and `CLAUDE_SEATS_DIR`, the missing-file line, the start line and both twins.

A seated session is never continued with `-c`. Profiles share the projects junction, so `-c` and `--continue` load the most recent conversation of the folder, which may belong to the other seat, and appending the right seat text to it does not stop a conversation that already holds a chair. The launcher refuses `-c`, `--continue` and `--continue=<id>` outright: it prints one line and exits without opening a session. Resume a seat with `-- -r` and the picker, `-- -r <session id>`, `-- -r<session id>` attached or `-- --resume=<session id>`.

The cap has one opt-in exception. `cc work -Window 260000` drops `CLAUDE_CODE_DISABLE_1M_CONTEXT` for that session and sets `CLAUDE_CODE_AUTO_COMPACT_WINDOW` to the number you give, so auto-compaction fires higher and the session compacts less often. Both variables are read by claude.exe, verified against 2.1.258, and the variable takes precedence over the `autoCompactWindow` setting. It is off unless you pass it, and it is a tradeoff, not a free win: a bigger window means a bigger floor on every turn and more expensive cache breaks (`docs/CONTEXT-ECONOMICS.md`, sections 2 and 4). A capped session also clears an inherited `CLAUDE_CODE_AUTO_COMPACT_WINDOW`, so a pane opened from a pane that used the switch starts at the cap instead of quietly keeping the window. `cc work -ShowEnv` prints the variables a launch would set, the inherited window if there is one, and the arguments it would hand to claude, then exits without opening a session, which is the way to check the wiring, and the way the tests do it. It also prints the seat plan: `COMMAND=`, the whole command line; `SEAT=`, `FRESH=` and `START=`; and `CLAUDE_BRIEFS_DIR=` and `CLAUDE_CLOSING_HOUR=`, which read `(unset)` on a role with no chair. The switch is named `-Window` and not `-CompactWindow` for a reason worth knowing before adding another parameter: PowerShell binds a parameter by unambiguous prefix, and this script forwards an unbound flag to claude, so a name starting with `c` would swallow claude's own `-c` and turn the cap off while the resume flag disappeared.

Two side notes. Every role except `research` adds `--no-chrome`, so a session that needs the browser either takes `research` or passes a `chrome` flag itself; it joins the argument array before the seat block, so the start line stays the last token and the in-window path receives it too. And `-Workspace <n>` (`-x`), with `-Tab`, picks the Herdr workspace for the new tab by number or id; it is ignored without Herdr.
