...
- `03-features/`: one spec per feature (F01 to F17), each mapping the FR/US it satisfies and carrying its own status line. F14 (quota wake), F15 (plugin marketplace, the second channel the skill sets reach a machine through), F16 (reserve mode, planning bought one stateless call at a time while the planning model is rationed) and F17 (vendored skills, three taken from a third-party library and rewritten for this kit) are implemented in the tree and stay proposals until they are reviewed and merged.
