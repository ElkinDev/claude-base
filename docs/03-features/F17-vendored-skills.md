...
- `install.ps1` running from a tree whose own origin it cannot read, a downloaded archive, a clone whose remote is not called origin, or a kit unpacked inside somebody else's clone, writes neither `extraKnownMarketplaces` nor `enabledPlugins` into the scaffolded settings, and says so in a warning naming both keys. Shipping the placeholders would point a project at a source that does not exist with a plugin enabled against it, which is worse than having no plugin source; shipping the surrounding repository's remote would be worse still, since a private slug and a foreign commit would land in a file the adopter is invited to commit. The remote is therefore trusted only when the repository git finds is this tree itself. The rest of the scaffold is unaffected and the run still succeeds.
