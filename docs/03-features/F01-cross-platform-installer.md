...
- Execute phase, per operation type: `copy` merges directories and backs up differing existing files as `.bak` while writing the incoming version as `.new` for manual merge, or overwrites with backup under `--force`; `skip-existing` copies only absent files; `render` substitutes `{HOME}`, `{GITBASH}`, `{BASE_BRANCHES}` and writes the result; `register` updates marked sections in instruction files and hook entries in settings, touching nothing outside markers. Ownership of an existing file is decided by the managed-file manifest, and every replacement is backed up first, exactly as F13 defines it; both ports read and write the same manifest and backup layout (`data-contracts.md`).
