...
Eight pieces, all zero-token, all in this repo:
...
| `claude/hooks/compact-recover.py` | SessionStart (compact), and on startup, resume, clear and fork with `--rulings` | both modes open with the seat block: `Seat: orchestrator`, `Seat: analyst` or `Seat: none (lane)` from `CLAUDE_ROLE`, the loud line when no launcher variable is in the environment at all, and the newest `<seat>-resume-*.md` in `CLAUDE_BRIEFS_DIR`. After compaction, injects the checkpoint path with its disk-truth section (capped at 900), then the open owner asks, then the rulings block, then the state sheet paragraph with its gates section, then the pointers to notes, brief and landings. The rulings block is the last 30 rows of the register, sorted by their stamp and capped at 7,200 characters inside a 9,000 character total, and it prints before everything the cap can cut, so a cut never takes a ruling; a register that is missing or empty says so in one line. The open owner asks are the rows of the newest decisions file by the date in its name (`~/.claude/decisions/owner-decisions-YYYY-MM-DD.md`, moved by `CLAUDE_DECISIONS_GLOB`) that no shut word closes, last written first, each clipped to 200 characters, at most 1,200 characters at a start and 450 after a compaction, where the file's name stands in for a path too long to leave room for the newest row; a subagent gets none, and with no such file nothing prints (docs/ADOPTION.md, Open owner asks). With `--rulings` the hook prints the seat block, the open asks and the rulings block and nothing else, and it does read stdin there, because the payload is what tells a subagent from the session that launched it and a subagent gets no seat block; a tty is never read, and every other stdin is read with a bounded wait of two seconds, so a hook run by hand prints at once and a caller that opens the pipe and stays silent costs the bound and not the session. That mode is what a SessionStart on startup, resume, clear and fork wires, and the settings template wires that entry now, so an adopter sees a rulings block at every session start and not only after a compaction. The three paths are home-relative defaults moved by `CLAUDE_RULINGS_FILE`, `CLAUDE_LANE_STATE_SCRIPT` and `CLAUDE_LANE_STATE_SHEET`. Facts only, no instruction to re-read anything, since that instruction is paid for on every compaction and is obeyed even when the summary already has the answer. |
| `claude/tools/lane-state.py` | a tool, not a hook | renders the state sheet the recovery hook points at, from files already on disk: the fixed lines, the worktrees of the configured repository, the last 30 rulings by stamp, the gate exit files of the last 24 hours, the tail of the landings file, and the lane reports and briefs touched today, cut to 170 lines from the bottom of the longest cuttable section so the fixed lines, the rulings and the gates always survive. A source that is missing prints one line saying why instead of failing the render, and the write is atomic, so a session that reads the sheet mid-render never sees half of one. Configured by `~/.claude/lane-state.json`; every key it does not set falls back to a path beside that file, and with no config file at all the fallbacks sit under `~/.claude` and nothing is scanned. `CLAUDE_LANE_STATE_CONFIG`, `CLAUDE_LANE_STATE_SHEET` and `CLAUDE_RULINGS_FILE` move the config, the sheet and the register. |
| `claude/hooks/prompt-log.py` | UserPromptSubmit, and SessionStart with `--recover` (compact, and startup, resume, clear and fork) | appends the prompt just submitted to `<checkpoints>/<session8>-prompts.md` as one stamped line, whitespace collapsed, cut at 1,500 characters, and trims the file to its last 200 KB. Prints nothing on submit, because a UserPromptSubmit hook's stdout becomes context, with one exception: a seated session inside the closing round of `CLAUDE_CLOSING_HOUR` gets one line saying the day ends and what it owes first. `--recover` prints the last ten entries back under a 6,000 character ceiling, verbatim, so an order given early in a session outlives the summary that paraphrases it; it runs after a compaction and at every other start, where a session with no log yet prints nothing (after a compaction it says there is none). Slash commands, harness tags and a subagent's prompt are not what a person typed and are skipped. |
...
full-context turn per gate ending (a mean of 275k tokens, 13 endings of 13), a low-memory guard
killed two of those background waits (426k per wake), and subagents that waited by re-arming
watchers re-sent 5.48M tokens in 48 turns.
...
morning (35 of 35, 2026-09-23), because a terminal multiplexer reads a session as working whenever
any of its subagents runs, which for an orchestrator is nearly always. Typing into a working pane
was worse: the screen read could not rule out a survey or a menu that the typed text would answer.
The rule that works splits the two cases:
