...
- Decisions are triaged before they reach the owner: anything decidable from a ruling, a law, precedent or a reversible default is decided by the orchestrator and logged in the landings file with the veto right stated; only product taste, visual design, money, legal exposure and the owner's own words go to the owner.
- What goes to the owner goes as a numbered list in the status block, never as "there are pending questions": each item is a one-line question with two to four labelled options, the recommended one marked with a one-line why, and what happens if it stays unanswered; the list is answerable as "1 A, 2 B, 3 all recommended" and is repeated in every status block until answered.
- A visual change is not ready to ask until its mockup exists in `<evidence-root>/mockups/` as `mockup-<id>-<slug>.html`, one file per change, the file path in the list item, so the owner can approve, reject or give feedback on the picture before any implementation starts; the root is resolved with `scripts/evidence-path.py --mockups --create` and never composed by hand.
