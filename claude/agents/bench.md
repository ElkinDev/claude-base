...
- A tap by label reads its coordinates from a screen dump taken after the last focus change, typing or navigation (an older dump puts the tap where the keyboard or the next screen now is), matches the label exactly, prefers the node whose text is the label over one whose accessibility label only equals it, and prints the ambiguity when more than one node matches instead of tapping the first; a blank label is refused.
