...
The review brief this seat writes by hand carries the table, one row per probe: `H<n> / entry point / scratch path / command / exit code / one output line`, or `H<n> / n/a / reason`. A fix or notes brief from `scripts/brief-gen.py` asks for the same table whenever its round writes or changes a script; the lane writes it into its report, and a review brief from the same script tells the reviewer to read that report whole.
