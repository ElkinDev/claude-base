...
- Before hand-back: one filtered run of the lane's OWN test classes (the pins and every test class the lane wrote or edited), never a whole suite; the gate runs the suites. The order: the commit, then the pre-check on the committed tip, then the report appended whole (the tip, the commit subject, the changes, the run tag and its .done path with the words "verdict in the .done", the pre-check line, the hostile-input table when a script changed, and Open items), then this run as the last tool call, as the line above says. Never hold the run log open (no tail -f, no editor).
